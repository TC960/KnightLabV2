...
def main():
    ...
    # ------------------------------------------- 5. the shippable layer
    # A SEPARATE, small, committed artifact rather than a new key in graph.json.
    # Three reasons: (a) whether disease subtypes should be modelled as
    # containment at all is a design call that belongs to a human, and an
    # opt-in file leaves that call reversible; (b) graph.json is regenerated by
    # build_kg.py, which would drop an externally-added key on the next build;
    # (c) it must not create a dependency on the 53 MB mondo.obo, which is not
    # committed and will not exist on most checkouts -- a retriever that
    # silently loses a capability when a data file is missing is the exact
    # failure mode that cost this repo 681 taxid resolutions while printing a
    # successful build.
    layer = {
        "mondo_version": m.version,
        "provenance": "MONDO is-a closure over disease labels resolved by mondo.py; "
                      "no hand-written parent-child assertions are included here. "
                      "The hand-written tiers in disease_containment.py cover "
                      "subtype labels MONDO does not carry and are deliberately "
                      "NOT shipped in this file.",
        "links": [{"child": f"d:{l['child']}", "parent": f"d:{l['parent']}",
                   "child_label": l["child"], "parent_label": l["parent"],
                   "child_mondo": l["child_mondo"], "parent_mondo": l["parent_mondo"],
                   "mondo_distance": l["mondo_distance"]} for l in links],
    }
    lpath = os.path.join(HERE, "disease_hierarchy_links.json")
    json.dump(layer, open(lpath, "w"), indent=1)
    print(f"\nwrote {lpath} ({len(links)} links, opt-in; graph.json untouched)")
