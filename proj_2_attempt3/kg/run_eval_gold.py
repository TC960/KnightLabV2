"""
run_eval_gold.py — copy of eval-v2/run_eval.py staged for the new-gold GPU run.

Three deltas from the original, all forced by what went wrong last time or by the
new gold file. Everything else (prompt, grammar, truncation, checkpointing) is
byte-identical, so results stay comparable to the leaderboard:

  1. DATASETS gains "goldset" -> kg/extract_input_gold.json (265 papers), and it
     is the default. The GPU copy of run_eval.py had an equivalent "newset" entry
     that was never committed; this file replaces that untracked edit.
  2. Scoring is wrapped in try/except and gated behind --score. The 2026-08-31 run
     extracted all 98 papers and then exited rc=1 on `ModuleNotFoundError: No
     module named 'sklearn'` while scoring -- the watchdog correctly read FAILED
     and deleted the box, and the results survived only because the checkpoint is
     flushed per paper. Extraction must not be able to fail after it has succeeded.
  3. A STATUS file is written next to the script (DONE / FAILED rc=N) so the
     watchdog's marker check works even if the shell wrapper dies.

One model per invocation (single GPU -> run sequentially, not in parallel).
    python run_eval_gold.py --model qwopus3.5-27b-v3
    python run_eval_gold.py --model qwopus3.5-27b-v3 --limit 2      # smoke test
...
_EMILY    = os.path.join(HERE, "..", "EmilySong_GoldStandardPaper")   # this copy lives in kg/, one level up from eval-v2/
...
DATASETS = {
    ...
    "goldset": os.path.join(HERE, "extract_input_gold.json"),
}

PROMPT_VERSION = "samgated-v1"
DATASET        = "goldset"
STATUS         = os.path.join(HERE, "STATUS")     # the watchdog polls this file
...
def main():
    ...
    ap.add_argument("--dataset", choices=list(DATASETS), default=DATASET,
                    help="goldset = 265 new-gold papers (default); testv2 = 15-paper benchmark; "
                         "all250 = full Emily corpus")
    ap.add_argument("--score", action="store_true",
                    help="score on the GPU box. Off by default: scoring needs sklearn + the NCBI "
                         "taxdump and is not worth risking a finished extraction. Score locally.")
    ...
    try:
        ...
        if not args.keep:
            cleanup_hf(repo)
        raise SystemExit(2)      # a load failure is FAILED, not DONE
    ...
    # ---- score ----
    # Extraction is DONE at this point and every row is on disk. Scoring needs
    # sklearn + the NCBI taxdump, neither of which the GPU image is guaranteed to
    # have; last run it did not, and the ModuleNotFoundError turned a complete
    # extraction into rc=1. Scoring is therefore opt-in and never fatal -- score
    # locally off the checkpoint instead.
    if not args.score:
        print("\nscoring skipped (--score to enable); extraction complete", flush=True)
        if not args.keep:
            cleanup_hf(repo)
        return
    try:
        _score(args, results, times, key, repo, quant, dataset)
    except Exception as e:
        print(f"\n!! scoring failed ({type(e).__name__}: {str(e)[:160]}) -- "
              f"extraction results are intact in {ckpt_path}", flush=True)
    if not args.keep:
        cleanup_hf(repo)


def _score(args, results, times, key, repo, quant, dataset):
...
if __name__ == "__main__":
    # The watchdog polls kg/STATUS and deletes the instance on DONE or FAILED.
    # Writing it here, not only in the shell wrapper, means a wrapper that dies
    # still cannot leave the GPU billing with no marker to read.
    try:
        main()
        open(STATUS, "w").write("DONE\n")
    except SystemExit as e:
        # argparse --help / --version raise SystemExit(0). Neither DONE nor FAILED
        # is right for those -- the watchdog deletes the instance on EITHER marker,
        # so `run_eval_gold.py --help` on the box would have killed the run. Leave
        # STATUS untouched on a zero exit; only a non-zero one is a real failure.
        code = e.code if isinstance(e.code, int) else (0 if e.code is None else 1)
        if code != 0:
            open(STATUS, "w").write(f"FAILED rc={code}\n")
        raise
    except BaseException as e:
        open(STATUS, "w").write(f"FAILED {type(e).__name__}: {str(e)[:200]}\n")
        raise
