...
All 25 automatic resolutions matched on a MONDO **primary name or an `EXACT`
synonym — none on a loose `RELATED` synonym**, which was checked before any id
was shipped. The 3 curated aliases are the only place judgement enters, and each
records its reason.

**Those ids are now in the graph.** `build_kg.py` consults a small committed
table (`disease_mondo_ids.json`, regenerated by `python3 mondo.py`) for labels
its own regex table does not cover, so **28 of 40 disease nodes carry a MONDO id
where 15 did before** — 254 further edges gained one, the largest being Spinal
cord injury (81), Intracerebral hemorrhage (50), multiple system atrophy and
CADASIL (20 each). `DISEASE_MAP` still wins outright where it has an entry,
including where its id is deliberately `None`, so the table cannot put an id back
on MCI. The builder warns loudly if the table is missing rather than silently
shipping a graph with fewer ids than the last one.

### A second bug in my own resolver, found by an arithmetic disagreement

The resolved count (27) disagreed with exact-matches-plus-aliases (28) by one.
Cause: curated aliases were consulted only when a label had **no** index hits,
but `CADASIL` *does* hit two MONDO terms (the general term and type 1) and was
therefore returned as `ambiguous`, silently ignoring its curated entry. Aliases
are now consulted whenever exact matching fails to yield exactly one live id.
Nothing flagged this except two numbers failing to agree — worth remembering as a
...
| `disease_mondo_ids.json` | committed label → id table `build_kg.py` consumes (no .obo dependency) |
| `disease_hierarchy_links.json` | the 2 shipped MONDO is-a links, opt-in, `graph.json` untouched |
