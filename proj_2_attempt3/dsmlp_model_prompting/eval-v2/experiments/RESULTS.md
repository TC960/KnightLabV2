...
taxonomy-aware metric vs both the original gold and the Opus 4.8 thorough benchmark.
...
| Qwopus3.5-27B (vs Opus 4.8) | 0.806 | 0.753 | 0.786 | 0.802 | — |
| Qwythos-9B (vs orig) | 0.665 | 0.590 | 0.644 | 0.665 | **0.809** |
| Qwythos-9B (vs Opus 4.8) | 0.742 | 0.646 | 0.650 | 0.740 | — |
| Qwen2.5-32B (vs orig) | 0.606 | 0.537 | **0.653** | 0.608 | **0.682** |
| Qwen2.5-32B (vs Opus 4.8) | 0.502 | 0.490 | 0.502 | 0.498 | — |

\* judge scored vs original gold, recovering metric-FPs the oracle (Opus 4.8 benchmark) confirms real:
...
1. **RELATE backfired on the local models** (Qwopus −.046, Qwythos −.075, Qwen −.069 vs original).
   This *reverses* the Opus 4.8 pilot. Cause: stage-1 retrieval was a crude regex (~200 noisy
...
3. **Judge (incomplete-gold recovery) is the biggest, most consistent lever** — +.13 to +.14 F1 on
   ...
   just Opus 4.8. (Caveat: oracle = Opus 4.8 benchmark, so this measures gold incompleteness, not an
...
- Keep **single-shot** extraction for the local models (Qwopus best local, F1 .806 vs Opus 4.8 gold).
